...
import 'package:hellonong/model/leg_sym.dart';
import 'package:hellonong/widget/bottomNavi.dart';

import '../home.dart';

class Picture_shin extends StatefulWidget {
  const Picture_shin({Key? key}) : super(key: key);

  @override
  State<Picture_shin> createState() => _Picture_shinState();
}
...
class _Picture_shinState extends State<Picture_shin> {
  ...
  List<Widget> _buildGridCards(BuildContext context) {
    List<Product> products = ProductsRepository.loadProducts(Category.leg);

    if (products.isEmpty) {
      return const <Widget>[];
    }

    List<Product> filteredProducts = products.where((product) => product.id.startsWith('5-')).toList();
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    AspectRatio(
                      ...
                      child: Image.asset(
                        //fit: BoxFit.fitWidth,
                        'assets/images/sym/shin/${product.id}.png', // 이미지 가져오기
                      ),
                    ),
                  ],
                ),
