...
import 'package:hellonong/model/arm_sym.dart';
import 'package:hellonong/widget/bottomNavi.dart';

import '../home.dart';

class Picture_shoulder extends StatefulWidget {
  const Picture_shoulder({Key? key}) : super(key: key);

  @override
  State<Picture_shoulder> createState() => _Picture_shoulderState();
}
...
class _Picture_shoulderState extends State<Picture_shoulder> {
  ...
  List<Widget> _buildGridCards(BuildContext context) {
    List<Product> products = ProductsRepository.loadProducts(Category.arm);

    if (products.isEmpty) {
      return const <Widget>[];
    }

    List<Product> filteredProducts = products.where((product) => product.id.startsWith('4-')).toList();
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    AspectRatio(
                      ...
                      child: Image.asset(
                        //fit: BoxFit.fitWidth,
                        'assets/images/sym/shoulder/${product.id}.png', // 이미지 가져오기
                      ),
                    ),
                  ],
                ),
